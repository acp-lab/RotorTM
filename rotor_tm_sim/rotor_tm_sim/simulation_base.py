...
from rotor_tm_sim.sim_utils import ptmassslackToTaut, ptmasstautToSlack, cooperativeGuard
import ipdb
...
class simulation_base(Node):
    ...
    def __init__(self,pl_params,uav_params):
        ...
        ################################## Peroforming  all the initialization####################################
        ...
        #subscribe wrenches from keyboard cmds
        self.create_subscription(WrenchMsg, '/payload/so3_control/ext_wrench',self.ext_wrench_callback, 10 )
        ...
        # Second Scenario: Point Mass
            ...
            x = np.zeros((26))            
            x[6] = 1.0
            x[15] = self.pl_params.cable_length
            x[19] = 1.0 
                                                
            
        ################################## Declaring publishers here #############################################
        ...
        for i in range(self.nquad):
            ...
            # ROS2 Publishers
            self.robot_odom_publisher.append(
                self.create_publisher(Odometry, f'{uav_name_with_id}/odom', qos_profile)
            )
            self.attach_publisher.append(
                self.create_publisher(Odometry, f'{uav_name_with_id}/attach', qos_profile)
            )

            # ROS2 Subscribers
            controller_name = f'/controller_{i + 1}'
            self.robot_command_subscriber.append(
                self.create_subscription(
                    RPMCommand,
                    f'{controller_name}/{uav_name_with_id}/rpm_cmd',
                    lambda msg, idx=i: self.rpm_command_callback(msg, idx),
                    qos_profile
                )
            )
            self.robot_command_subscriber.append(
                self.create_subscription(
                    FMCommand,
                    f'{controller_name}/{uav_name_with_id}/fm_cmd',
                    lambda msg, idx=i: self.fm_command_callback(msg, idx),
                    qos_profile
                )
            )
        ...
        ##set params
        try:
            self.hybrid_flag = self.get_parameter("hybrid_switch").get_parameter_value().bool_value
                       
        except:
            #Declare and set the parameter
            self.declare_parameter("hybrid_switch", False)
    ...
    def timer_callback(self):
          ...
          if self.hybrid_flag:
            ...
            if self.pl_params.id == "Rigid Link":
                sol = scipy.integrate.solve_ivp(self.rigid_links_cooperative_rigidbody_pl_EOM, self.t_span, self.x, method= 'RK45', t_eval=self.t_span)
                self.x = sol.y[:,-1]
            else:                            
                # Second Scenario: Point Mass
                if self.nquad == 1:
                    ...
                    pl_pos = self.x[0:3]
                    pl_vel = self.x[3:6]
                    robot_pos = self.x[13:16]
                    robot_vel = self.x[16:19]
                    ...
                    if cable_norm_vel.all() > 1e-3 and not self.cable_is_slack.all():
                        print("Colliding")
                        v1, v2 = self.ptmass_inelastic_cable_collision(self.x[0:6], self.x[13:19], self.pl_params.mass, self.uav_params[0].mass)
                        self.x[3:6] = v1
                        self.x[16:19] = v2
                    ...
                    if self.cable_is_slack:
                        ...
                        #print(self.x)
                        sol = scipy.integrate.solve_ivp(self.hybrid_ptmass_pl_transportationEOM, self.t_span, self.x, method= 'RK45', t_eval=self.t_span, events=ptmassslackToTaut)
                    else:
                        #print("Cable is taut")
                        #print(self.x)
                    ...
                    self.cable_is_slack = self.isslack(self.x[0:3], self.x[13:16], self.pl_params.cable_length)
                ...
                # First Scenario: Cooperative 
                    ...
                    inelastic_collision_flag, xi = self.cooperative_check_inelastic(self.x)
                    ...
                    while np.any(inelastic_collision_flag):
                        ...
                        self.x = self.rigidbody_quad_inelastic_cable_collision(self.x, inelastic_collision_flag)
                        print("collision finished!")
                        after_collide_inelastic_collision_flag, xi = self.cooperative_check_inelastic(self.x)
                        ...
                        if np.any((after_collide_inelastic_collision_flag - before_collide_inelastic_collision_flag)>0):
                            ...
                            attach_point_position = self.x[0:3].reshape((3,1))+ utilslib.QuatToRot(self.x[6:10]) @ self.pl_params.rho_vec_list
                            inelastic_collision_flag = after_collide_inelastic_collision_flag
                            for i in range(self.nquad):
                                print("The robot ", i+1, " position is ", self.x[(i+1)*13:(i+1)*13+3])
                                print("The xi ", i+1, " is ", xi[:,i])
                                print("The new robot position is, ", attach_point_position[:,i] - self.pl_params.cable_length[i] * xi[:,i])
                                self.x[(i+1)*13:(i+1)*13+3] = attach_point_position[:,i] - self.pl_params.cable_length[i] * xi[:,i]
                    ...
                    GuardEvents = []
                    for i in range(self.nquad):
                        GuardEvents.append(lambda t, x_val: cooperativeGuard(t, self.x, GuardEvents[i].pl_params.nquad, GuardEvents[i].slack_condition, GuardEvents[i].pl_params.rho_vec_list, GuardEvents[i].pl_params.cable_length, GuardEvents[i].i))
                    ...
                    ## state integration
                    sol = scipy.integrate.solve_ivp(self.hybrid_cooperative_rigidbody_pl_transportationEOM, self.t_span, self.x, method='RK23', t_eval=self.t_span, events=GuardEvents)
                    ...
                    if EventTriggered_bool == 1:
                        for i in range(self.nquad):
                            if len(sol.y_events[i]) != 0: 
                                EventTriggered_id = i
                        if (np.all(self.x==sol.y[:, -1])):
                            self.x = sol.y_events[EventTriggered_id][:]
                            self.x = self.x.T
                            self.x = self.x.reshape((self.x.shape[0],))
                    else:
                        self.x = sol.y[:,-1]
                        # print("Getting self.x here: ", self.x)
                    
                    self.cable_is_slack = self.isslack_multi(self.x[0:3].reshape((3,1))+ utilslib.QuatToRot(self.x[6:10]) @ self.pl_params.rho_vec_list, self.x[13*np.arange(1, self.nquad+1)+np.array([[0],[1],[2]])],self.pl_params.cable_length)
            ...
            payload_odom.twist.twist.linear.x    = self.x[3]
            payload_odom.twist.twist.linear.y    = self.x[4]
            payload_odom.twist.twist.linear.z    = self.x[5]
            payload_odom.pose.pose.orientation.w = self.x[6] 
            payload_odom.pose.pose.orientation.x = self.x[7]
            payload_odom.pose.pose.orientation.y = self.x[8]
            payload_odom.pose.pose.orientation.z = self.x[9]
            ...
            for uav_id in range(self.nquad-1, -1, -1):
                if self.pl_params.mechanism_type == 'Rigid Link':
                    ...
                    uav_state = self.x[13:26]
                    attach_pos = self.x[0:3] + payload_rotmat @ (self.pl_params.rho_robot[:,uav_id]+np.array([0.0,0,0.04]))
                    ...
                    uav_state = self.x[self.pl_dim_num+self.uav_dim_num*uav_id:self.pl_dim_num+self.uav_dim_num*(uav_id+1)]
                    attach_pos = self.x[0:3] + np.matmul(payload_rotmat, self.rho_vec_list[:,uav_id])
                    attach_vel = self.x[3:6] + np.matmul(payload_rotmat, np.cross(sol.y[:,0][10:13], self.rho_vec_list[:,uav_id]))
                    if not self.cable_is_slack[uav_id]:
                        uav_attach_vector = uav_state[0:3] - attach_pos[0:3]
                        uav_attach_distance = np.linalg.norm(uav_attach_vector)
                        if uav_attach_distance > self.cable_len_list[uav_id]:
                ...
                white_noise = np.random.multivariate_normal(np.zeros(12),self.uav_white_noise_cov)
                deltaR = utilslib.expSO3(white_noise[6:9])
            ...
            system_marker.markers.append(rosutilslib.update_marker_msg(self.payload_marker_msg,self.x[0:3],self.x[6:10],self.nquad+2))
            ...
            for uav_id in range(self.nquad-1, -1, -1):
                if self.pl_params.mechanism_type == 'Rigid Link':
                    uav_state = self.x[13:26]
                    attach_pos = self.x[0:3] + payload_rotmat @ (self.pl_params.rho_robot[:,uav_id]+np.array([-0.06,0,0.02]))
                    uav_state[0:3] = attach_pos
                    attach_vel = uav_state[3:6] + np.matmul(payload_rotmat, np.cross(sol.y[:,0][10:13], self.pl_params.rho_robot[:,uav_id]))
                    if not self.cable_is_slack[uav_id]:
                        uav_attach_vector = uav_state[0:3] - attach_pos[0:3]
    ...
    def rigidbody_payloadEOM_readonly(self, s, F, M, invML, C, D, E): 
        ...
        # print("printing dynamic model of a payload")
        # print("C :",C)
        # print("D", D)
        # print("E", E)
