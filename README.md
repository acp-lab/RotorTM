...
$ mkdir RotorTM-NMPC_WS
$ cd RotorTM-NMPC_WS
$ git clone --branch NMPC git@github.com:acp-lab/RotorTM.git
...
```
By default, the `editable_launch.py` is set to initialize the 3 MAVs cable suspended triangular payload. Directly editing the `editable_launch.py` allows various scenarios to be initialized. A Comprehensive list of all possible scenarios is provided as comments inside `editable_launch.py`. Please refer to that in the `editable_launch.py`.
### Start simulation
