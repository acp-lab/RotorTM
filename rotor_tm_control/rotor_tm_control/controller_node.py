...
def extract_number(uav_name):
    # Use regular expression to find digits at the end of the string
    match = re.search(r'\d+$', uav_name)  # Matches one or more digits at the end
...
class controller_node(Node):
    ...
    def __init__(self, node_id, single_node, payload_params_path, uav_params_path, mechanism_params_path, payload_control_gain_path, uav_control_gain_path):
        ...
        if self.single_node:
            ...
            for uav_id in range(self.pl_params.nquad):
                ...
                self.qd[uav_id] = {}
                self.create_subscription(Odometry, mav_odom, lambda msg: self.qd_odom_callback(msg,uav_id), qos_profile)
                
            
            # init ROS Publishers
            self.cen_pl_cmd_pub = self.create_publisher(CenPLCommand,"/payload/cen_pl_cmd", 10 )
            ...
            #print(uav_odom)
            self.qd[node_id] = {}
            self.create_subscription( Odometry, uav_odom, lambda msg: self.qd_odom_callback(msg,node_id), qos_profile)
                
            # init ROS Publishers
            FM_message_name = '/' + uav_name + "/fm_cmd"
            des_odom_name = '/' + uav_name + "/des_odom"
            self.cen_pl_cmd_pub = self.create_publisher(CenPLCommand, node_name + "/payload/cen_pl_cmd",10)
            self.cen_pl_wrench_pub = self.create_publisher(Wrench, node_name + "/payload/cen_wrench_cmd", 10)
            self.FM_pub.append(self.create_publisher(FMCommand, node_name +  FM_message_name, qos_profile))
            self.status_pub = self.create_publisher(Bool, node_name + "/heartbeat" , 10)
    ...
    def assembly_FM_message(self, F_list, M_list, uav_id):
        ...
        if self.pl_params.mechanism_type == 'Rigid Link':
            ...
            FM_message = FMCommand()
            FM_message.header.stamp = self.clock.now().to_msg()
            if self.pl_params.payload_type == 'Rigid Body':
                FM_message.thrust = F_list[uav_id].item()
                FM_message.moments.x = M_list[uav_id][0].item()
                FM_message.moments.y = M_list[uav_id][1].item()
                FM_message.moments.z = M_list[uav_id][2].item()
                return FM_message
            elif self.pl_params.payload_type == 'Point Mass':
                #print(M_list)
                FM_message.thrust = F_list[0,0]
                FM_message.moments.x = M_list[0,0]
                FM_message.moments.y = M_list[1,0]
                FM_message.moments.z = M_list[2,0]
                return FM_message
    ...
    def qd_odom_callback(self, uav_odom, uav_id):
        ...
        # topic_name = self.subscriptions.get_topic_name()  # Get the node name      
        # uav_name = topic_name.split('/')[0]  # This will give you 'dragonfly1'
        # uav_id = extract_number(uav_name)
        # print(topic_name)
        #print(uav_id)
        #print("@@@@@@@@@@")
        #print(uav_id)
...
def main():
    ...
    # if int(sys.argv[2]) == 1:
    if int(single_node) == 1:
        control_node = controller_node(int(nr), True, payload_params_path, uav_params_path, mechanism_params_path, payload_control_gain_path, uav_control_gain_path)
    else:
